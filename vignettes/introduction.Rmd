...
shows how to get data from a GenIE experiment targeting rs6700034, which is a 5' UTR SNP in gene
MUL1. It downloads 8 replicates for cDNA and 4 replicates for gDNA - about
57 Mb in a total of 24 files (12 ".bam" files and 12 ".bam.bai" files).

```{r download_example, message=FALSE}
library(rgenie)
library(tidyverse, quietly = T) # Not required, but we use it below
...
```{r}
print(regions$wt_allele_profile)
print(regions$hdr_allele_profile)
print(regions$ref_sequence)
```
...
```{r}
replicates = replicates %>% filter(replicate != "c1.1")
```
...
The deletion analysis also has many more output tables. We encourage you to explore these outputs yourself, which are described in more detail in the **Advanced rgenie** vignette.

The `region_stats` for a deletion_analysis differs from that for a grep_analysis. Although the result is a tidyverse tibble, we show it here in list format to that the results are easier to see.

```{r}
tbl = tibble(Name = names(del_result$region_stats), Value = as.character(del_result$region_stats))
knitr::kable(tbl)
```
...
The below plot summarizes the overall statistics for the region, and values for each replicate. (When there are many replicates the data may not fit, in which case you should manually look at the result$replicate_stats table)
...
The replicate QC plot is discussed in more detail in the **Advanced rgenie** vignette. Broadly, it computes some statistics that help to determine whether any samples are outliers, based on the deviation of some of the top deletion alleles from the average.
...
```{r fig.width=7, fig.height=6}
rgenie::experiment_summary_plot(grep_results, del_results)
```
...
```{r fig.width=7, fig.height=6}
grep_tables = rgenie::bind_results(grep_results)
names(grep_tables)

del_tables = rgenie::bind_results(del_results)
names(del_tables)
```
...
You may also wish to consult the **Advanced rgenie** vignette.
