...
# Note that to get the vignette to pass CRAN's checks, we can't download data
...
setwd("~/genie_example/MUL1/")
mul1_del_results = rgenie::alignment_analysis(mul1_regions,
                                             mul1_replicates,
                                             required_match_left = 10,
                                             required_match_right = 10,
...
The `region_stats` for an alignment_analysis differs from that for a grep_analysis. Although the result is a tidyverse tibble, we show it here in list format so that the results are easier to see.
...
```{r fig.width=7, fig.height=6}
rgenie::alignment_summary_plot(del_result)
del_result$replicate_stats
```

The statistics on the right hand side show (at the top) the estimated average effect size of deletions that appear anywhere in the amplicon. The statistics below this on the right hand side show the estimated average effect size of deletions that are strictly within the window indicated. (I.e. deletions that are partly within this window are not included.)
...
The bottom plot row shows the ***unique alleles***, with a horizontal bar depicting the region deleted. If the `color_by="window"` parameter is specified, then deletions which are contained within a given window (defined by `del_span_start/end` parameters in the call to alignment_analysis) are highlighted in red.
...
You may often want to just get all of the available plots for a deletion analysis. You can do this by calling `alignment_analysis_plots`.

```{r fig.width=7, fig.height=6}
all_plots = rgenie::alignment_analysis_plots(del_result,
...
Note that if you pass in a list of alignment_analysis results for multiple regions, then you will get plots for all of the regions.
...
This is the reason that the `grep_analysis` and `alignment_analysis` functions take in a table of regions and replicates as input. Each returns a list as a result, after conducting the analysis separately for each region. rgenie provides a plot to view a summary of results across many regions.
...
You may also wish to consult the [rgenie in depth vignette](https://htmlpreview.github.io/?https://github.com/Jeremy37/rgenie/blob/genie_atac/vignettes/advanced_rgenie.html) (available online).

For applying GenIE to ATAC data, see the [rgenie for ATAC vignette](https://htmlpreview.github.io/?https://github.com/Jeremy37/rgenie/blob/genie_atac/vignettes/rgenie_atac.html) (available online).
